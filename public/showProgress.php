...
$sql="create temporary table optikon.testscores(id varchar(255), avgscore int, primary key(`id`))";
 ...
 $sql="create temporary table dodo select dolly.*, round((least(wordscore, 500))/5) as word, (least(MathPerc, 100)) AS math, round(least(avgrepnum * 10, 50) + least(numLearning, 50)) AS activity,
 round(((avgscore +  round((least(wordscore, 500))/5) + (least(MathPerc, 100)) + round(least(avgrepnum * 10, 50) + least(numLearning, 50)))/ 4)) AS total from dolly";
